...
describe('handlers', () => {
  describe('combineHandlers', () => {
    it('should create actions', () => {
      ...
      expect(fetchBooks).toEqual({ type: '[books] fetchBooks' });

      const fetchBooksSuccess = actions.fetchBooksSuccess({ books: ['B1'] });
      expect(fetchBooksSuccess).toEqual({ type: '[books] fetchBooksSuccess', books: ['B1'] });

      const createBook = actions.createBook({ book: 'B1' });
      expect(createBook).toEqual({ type: '[books] createBook', book: 'B1' });

      const showCreateBookDialog = actions.showCreateBookDialog();
      expect(showCreateBookDialog).toEqual({ type: '[books] showCreateBookDialog' });
    });
    ...
    it('should create a reducer', () => {
      ...
      const state5 = reducer(initialState, { type: '[unknown] fetchBooks' });
      expect(state5).toEqual(initialState);

      const state6 = reducer(initialState, { type: '[unknown] unknown' });
